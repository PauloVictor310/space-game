...
//
// Descrição:   Graphics Euma classe que faz uso das funções do Direct3D para 
//              acessar o hardware de aceleração gráfica da placa de vúeo.
//
**********************************************************************************/
...
Graphics::~Graphics()
{
    ...
    if (swapChain)
    {
        // Direct3D Eincapaz de fechar quando em tela cheia
...
bool Graphics::Initialize(Window * window)
{
    ...
    if FAILED(
        D3D11CreateDevice(
            NULL,                           // adaptador de vúeo (NULL = adaptador padrão)
    ...
    // cria objeto para adaptador de vúeo (placa gráfica)
    ...
    swapDesc.BufferDesc.RefreshRate.Denominator = 1;            // numerador Eum inteiro e não um racional
    ...
    swapDesc.SampleDesc.Quality = 0;                            // nú“el de qualidade da imagem
    swapDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;     // utilize superfú€ie como RENDER-TARGET
    ...
    swapDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;        // descarta superfú€ie após apresentação
    ...
    // pega a superfú€ie backbuffer de uma swapchain
    ...
    // Combinando superfú€ies transparentes (Alpha Blending)
    ...
    blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA;        // destino da mistura RGB Eo alpha invertido 
    blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;                 // operação de adição na mistura de cores
    blendDesc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;              // fonte da mistura Alpha Eo alpha do pixel shader
    blendDesc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;            // destino da mistura Alpha Eo alpha invertido
